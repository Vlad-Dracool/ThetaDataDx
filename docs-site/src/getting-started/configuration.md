...
```bash
tdx stock eod AAPL 20240101 20240301 --config production  # default
tdx stock eod AAPL 20240101 20240301 --config dev
```
...
The `mdds_concurrent_requests` field controls the maximum number of in-flight gRPC requests to the MDDS server. Each endpoint method acquires a semaphore permit before sending and releases it when the response is fully consumed.
...
The auto-detection reads your subscription tier from the Nexus auth response, matching the Java terminal's concurrency model.
